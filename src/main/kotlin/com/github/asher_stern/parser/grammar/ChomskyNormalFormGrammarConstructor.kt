...
)
{
    fun construct(): ChomskyNormalFormGrammar<N, T>
    {
        for (rule in grammar.rules)
        {
            ...
            {
                singleToSingleRules[rule.rhs.first().symbol!!, rule.lhs] = rule.logProbability
            }
        ...
        return ChomskyNormalFormGrammar(grammar.startSymbol, grammar.terminals, grammar.nonTerminals, terminalRules, singleToSingleRules, nonTerminalRules)
    }
    ...
    private val singleToSingleRules = Table2D<N, N, Double>() // from rhs to lhs to log probability
}